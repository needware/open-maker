...
```bash
cd /c/Users/ekada/OneDrive/Desktop/Githubcontributing/open-make
git status
...
```bash
pnpm --filter @open-make/web build 2>&1 | tail -20 > /tmp/web-baseline-build.txt
```
...
```bash
pnpm --filter @open-make/contracts test critique.test.ts
```
...
```bash
pnpm --filter @open-make/contracts test sse.test.ts
```
...
```bash
pnpm --filter @open-make/contracts test
```
...
import type { PanelEvent } from '@open-make/contracts/critique';
...
```bash
pnpm --filter @open-make/daemon test parser.test.ts
```
...
import type { PanelEvent, PanelistRole } from '@open-make/contracts/critique';
...
import { describe, expect, it } from 'vitest';
import { defaultCritiqueConfig } from '@open-make/contracts/critique';
...
```bash
pnpm --filter @open-make/daemon test scoreboard.test.ts
```
...
import type { PanelistRole } from '@open-make/contracts/critique';
...
```ts
import type { CritiqueConfig, RoundDecision } from '@open-make/contracts/critique';
...
```ts
import type { FallbackPolicy } from '@open-make/contracts/critique';
...
```bash
pnpm --filter @open-make/daemon test migrations.test.ts
```
...
```bash
pnpm --filter @open-make/daemon test orchestrator.test.ts
```
...
} from '@open-make/contracts/critique';
import { panelEventToSse } from '@open-make/contracts/sse';
import { parseCritiqueStream } from './parser';
...
```ts
import { describe, expect, it } from 'vitest';
import { defaultCritiqueConfig, PROTOCOL_VERSION } from '@open-make/contracts/critique';
...
import { type CritiqueConfig, PROTOCOL_VERSION } from '@open-make/contracts/critique';
...
```bash
pnpm --filter @open-make/web test discovery.test.ts
```
...
import type { CritiqueSseEvent } from '@open-make/contracts/sse';
import type { PanelistRole } from '@open-make/contracts/critique';
