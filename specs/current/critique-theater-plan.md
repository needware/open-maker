...
```bash
cd /c/Users/ekada/OneDrive/Desktop/Githubcontributing/open-maker
git status
...
```bash
pnpm --filter @open-maker/web build 2>&1 | tail -20 > /tmp/web-baseline-build.txt
```
...
```bash
pnpm --filter @open-maker/contracts test critique.test.ts
```
...
```bash
pnpm --filter @open-maker/contracts test sse.test.ts
```
...
```bash
pnpm --filter @open-maker/contracts test
```
...
import type { PanelEvent } from '@open-maker/contracts/critique';
...
```bash
pnpm --filter @open-maker/daemon test parser.test.ts
```
...
import type { PanelEvent, PanelistRole } from '@open-maker/contracts/critique';
...
import { describe, expect, it } from 'vitest';
import { defaultCritiqueConfig } from '@open-maker/contracts/critique';
...
```bash
pnpm --filter @open-maker/daemon test scoreboard.test.ts
```
...
import type { PanelistRole } from '@open-maker/contracts/critique';
...
```ts
import type { CritiqueConfig, RoundDecision } from '@open-maker/contracts/critique';
...
```ts
import type { FallbackPolicy } from '@open-maker/contracts/critique';
...
```bash
pnpm --filter @open-maker/daemon test migrations.test.ts
```
...
```bash
pnpm --filter @open-maker/daemon test orchestrator.test.ts
```
...
} from '@open-maker/contracts/critique';
import { panelEventToSse } from '@open-maker/contracts/sse';
import { parseCritiqueStream } from './parser';
...
```ts
import { describe, expect, it } from 'vitest';
import { defaultCritiqueConfig, PROTOCOL_VERSION } from '@open-maker/contracts/critique';
...
import { type CritiqueConfig, PROTOCOL_VERSION } from '@open-maker/contracts/critique';
...
```bash
pnpm --filter @open-maker/web test discovery.test.ts
```
...
import type { CritiqueSseEvent } from '@open-maker/contracts/sse';
import type { PanelistRole } from '@open-maker/contracts/critique';
