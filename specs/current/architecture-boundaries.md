...
This document defines the architectural boundaries for the local Open Maker app. These boundaries are architectural constraints; some enforcement details can be implemented later through the relevant roadmap workstreams.

## Product Shape

Open Maker is a local-first application. The near-term Electron version is a shell around the same `apps/web` and `apps/daemon` architecture.
