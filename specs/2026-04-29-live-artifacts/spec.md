...
This spec defines how to bring Monet's **connectors** and **live artifacts** ideas into Open Maker, but implement the agent-facing surface as **file-based agent skills plus daemon-owned local tools**, not as an in-process tool registry or MCP-first integration.

---

## 1. Product goal

Open Maker should let an agent create previewable artifacts that are not just one-off generated files, but **live, refreshable, auditable views** backed by external or local data sources.
...
name: live-artifact
description: |
  Create refreshable, auditable Open Maker artifacts backed by connector or local data.
...
- OAuth connection state and credentials live outside project artifacts, under a daemon-controlled global store such as `~/.open-maker/connectors/` or an app database.
