...
- Local daemon (Node)
  ...
  - **project model: project ≡ a folder on disk** (no UUID-keyed shadow tree); registry keyed by `realpath`, persisted as `~/.open-maker/recent-projects.json`
...
4. Deployed example at `demo.open-maker.dev` (Topology C).
