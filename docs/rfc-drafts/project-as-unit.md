...
| 4 | **Projects do not see each other** | No cross-project sources, no shared KB, no shared facets. `~/.open-design/recent-projects.json` is a UI list only — not a context source. |
...
| 6 | **Outputs stay inside the project** | Facet artifacts write to `<project-root>/artifacts/<facet-id>/`. Writing outside the project root (to `/tmp/`, `~/.open-design/`, another project) is a bug. |
...
For the open-design repository itself, no `KNOWLEDGE.md` is required: the existing `docs/` directory is auto-detected as the source root, making this repo a working example project on first open.
...
```json
{
  ...
  "lastBrief": "SaaS landing for Open Design; hero highlights skills protocol.",
...
└─ [Open folder…]   → native folder picker → daemon resolves realpath
                    → adds to ~/.open-design/recent-projects.json
...
`~/.open-design/recent-projects.json`:
...
```json
{
  ...
  "entries": [
    ...
    { "path": "/Users/me/code/open-design", "lastOpenedAt": "2026-05-09T20:14:11Z" }
  ]
}
```
...
| `GET /api/projects/recent` | New | Reads `~/.open-design/recent-projects.json` |
...
Shared DTOs added to `@open-design/contracts`:
...
- `apps/daemon`: project registry keyed by realpath, facet store, source resolver, ToC indexer, sibling-facet summarizer, recent-projects file at `~/.open-design/recent-projects.json`.
...
After this RFC ships, opening `/Users/<you>/code/open-design/` in OD does the following:

1. `POST /api/projects/open { path: "/Users/<you>/code/open-design" }`.
...
7. User types: *"a SaaS-landing prototype that explains Open Design to indie developers, hero pulls from spec.md."*
...
9. Agent runs with `cwd = /Users/<you>/code/open-design`, reads `docs/spec.md` and `docs/skills-protocol.md`, writes to `artifacts/prototype-landing/index.html`.
