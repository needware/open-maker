...
interface Props {
  ...
  // boundary). Forwarded through to NewProjectModal → NewProjectPanel
  ...
  onImportFolderResponse?: (
    response: import('@open-design/contracts').ImportFolderResponse,
  ) => Promise<void> | void;
...
export function EntryShell({
  ...
  async function handleChipFolderImport() {
    ...
    // pick + HMAC-gated import). When that bridge is present and we
    // have an `onImportFolderResponse` callback wired, dispatch the
    // atomic pick + import in one step so the user lands in the new
    // project without seeing the New Project modal.
    const hasElectronPickAndImport =
      typeof window !== 'undefined' &&
      typeof (window as any).electronAPI?.pickAndImport === 'function';
    if (hasElectronPickAndImport && onImportFolderResponse) {
      try {
        const result = await (window as any).electronAPI.pickAndImport({
          source: 'home-chip',
        });
        ...
        console.error('home-chip pickAndImport failed', err);
      }
      return;
    }
