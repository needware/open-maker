...
// open-design issue URL and belongs to the "Open Maker PR" button. Before
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to open-design`. That command emits an open-design issue URL and belongs to the "Open Maker PR" button.',
    '',
    'Run the steps below in order. Report each command and its result. Stop on the first hard failure — do not retry blindly.',
    '',
