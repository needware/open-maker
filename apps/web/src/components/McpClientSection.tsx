...
//
// Open Maker connects to the configured servers as a CLIENT and surfaces
...
// so the most useful categories for Open Maker (visual generation, then
...
export const McpClientSection = forwardRef<McpClientSectionHandle, Props>(
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled) return;
      if (!data) {
        setError('Could not reach the local daemon. Make sure Open Maker is running, then reopen this panel.');
        setLoaded(true);
        return;
      }
        ...
        <>
          ...
          {isHttpLike && row._isNew ? (
            <div className="mcp-oauth-hint hint">
              Save first, then click <strong>Connect</strong> to grant Open Maker
        ...
        {connected ? (
          ...
          <>
            ...
            <span>
              ...
              <span className="hint">
                Click Connect to grant Open Maker access via the provider's OAuth flow.
              </span>
            </span>
          </>
        )}
