...
open-maker-landing skill. Several common image-model defaults will
...
See `skills/open-maker-landing/assets/imagegen-prompts.md` for the
...
- Always include the metadata strip, the side rails, the Roman
  section rules, and a footer with a giant `Open Maker.` (or brand)
