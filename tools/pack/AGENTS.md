...
- Local packaging orchestration for packaged Open Make artifacts.
...
- Consuming sidecar/process/path primitives from `@open-make/sidecar-proto`, `@open-make/sidecar`, and `@open-make/platform`.
...
- Release artifacts keep canonical app identity (`Open Make.app` on mac, `Open Make.exe` inside the Windows installer); local tools-pack installs may use namespace-scoped install paths only as a developer multi-instance validation convention.
