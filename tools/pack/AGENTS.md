...
- Consuming sidecar/process/path primitives from `@open-maker/sidecar-proto`, `@open-maker/sidecar`, and `@open-maker/platform`.
...
- Release artifacts keep canonical app identity (`Open Maker.app` on mac, `Open Maker.exe` inside the Windows installer); local tools-pack installs may use namespace-scoped install paths only as a developer multi-instance validation convention.
