...
- `packages/sidecar-proto`: Open Maker sidecar business protocol. Owns app/mode/source constants, namespace validation, stamp descriptor/fields/flags, IPC message schema, status shapes, error semantics, and default product path constants.
- `packages/sidecar`: generic sidecar runtime primitives. Includes bootstrap, IPC transport, path/runtime resolution, launch env, and JSON runtime file helpers; it must not hard-code Open Maker app keys or IPC business messages.
...
- Do not hard-code Open Maker app/source/mode constants in `sidecar` or `platform`.
- Keep stamp fields limited to five: `app`, `mode`, `namespace`, `ipc`, and `source`.

## Common package commands

```bash
pnpm --filter @open-maker/contracts typecheck
pnpm --filter @open-maker/sidecar-proto typecheck
pnpm --filter @open-maker/sidecar-proto test
pnpm --filter @open-maker/sidecar typecheck
pnpm --filter @open-maker/sidecar test
pnpm --filter @open-maker/platform typecheck
pnpm --filter @open-maker/platform test
```
