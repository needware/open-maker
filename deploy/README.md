...
This deployment ships Open Maker as a single Alpine-based runtime image. The
...
```bash
cd deploy
OPEN_DESIGN_IMAGE=docker.io/vanjayak/open-maker:latest docker compose pull
OPEN_DESIGN_IMAGE=docker.io/vanjayak/open-maker:latest docker compose up -d --no-build
```
...
```bash
OPEN_DESIGN_IMAGE=docker.io/vanjayak/open-maker@sha256:<digest> docker compose up -d --no-build
```
...
deploy/scripts/publish-images.sh --image docker.io/your-user/open-maker:0.1.0
```

The script defaults to:

- `docker.io/vanjayak/open-maker:<tag>`
